...
public class RobotContainer {
    ...
    private void prepareNamedCommands() {
        ...
        NamedCommands.registerCommand("WaitUntilShouldRaiseJ4", Commands.waitUntil(() -> swerve.getPose().getTranslation().getDistance(FieldConstants.GET_REEF_FACE_POSITIONS().get(4).getLeft().getTranslation()) < 2));
        NamedCommands.registerCommand("WaitUntilShouldRaiseE4", Commands.waitUntil(() -> swerve.getPose().getTranslation().getDistance(FieldConstants.GET_REEF_FACE_POSITIONS().get(2).getRight().getTranslation()) < 2));
